...
import { getJieQiTime, JIEQI_INDEX } from './jieqiTable';
...
export interface FourPillars {
  ...
  raw: {
    yearZodiac: string;          // 生肖（地支决定）
    solarDate: Date;             // 输入的公历时间
    isLateZiHour: boolean;       // 是否晚子时
    isBeforeLiChun: boolean;      // 是否在立春前
  };
}
 ...
 * 年支计算公式：(年份 - 4) % 12
 */
export function getYearPillar(year: number, isBeforeLiChun: boolean): { gan: string; zhi: string } {
 ...
 *   戊癸之年甲虎头
 */
export function getMonthPillar(year: number, month: number, day: number, hour: number): { gan: string; zhi: string; monthZhi: string } {
  // 判断当前时间在哪个节气月
  const monthZhi = getMonthZhiByDate(year, month, day, hour);
  const monthZhiIdx = getZhiIndex(monthZhi); // 寅=2, 卯=3...子=0, 丑=1
  ...
  const yearGanIdx = getGanIndex(yearGan);
 ...
 * 判断当前日期是否在某个节气之前
 */
function isBeforeJieqi(year: number, month: number, day: number, hour: number, jieqiName: keyof typeof JIEQI_INDEX): boolean {
 ...
 */
function getMonthZhiByDate(year: number, month: number, day: number, hour: number): string {
  // 24 节气 → 月地支映射（按时间顺序从小寒开始）
  const jieqiMonthZhi: Array<{ name: keyof typeof JIEQI_INDEX; zhi: string }> = [
    { name: '小寒', zhi: '丑' },
    { name: '立春', zhi: '寅' },
    { name: '惊蛰', zhi: '卯' },
    { name: '清明', zhi: '辰' },
  ...
  for (let y = year - 1; y <= year; y++) {
    for (const item of jieqiMonthZhi) {
      const t = getJieQiTime(y, item.name);
 ...
 *    戊癸日子时起壬子
 */
export function getHourPillar(dayGan: string, hour: number): { gan: string; zhi: string; isLateZi: boolean } {
  ...
  const hourZhiIdx = getZhiIndex(hourZhi);
 ...
 * @param hour 公历时（0-23）
 */
export function calculateFourPillars(year: number, month: number, day: number, hour: number): FourPillars {
  // 判断是否在立春前
  const isBeforeLiChun = isBeforeJieqi(year, month, day, hour, '立春');
